...
data:any): Promise<ws.IPipeline> => {
    ...
    let pipeline = await init.device.createRenderPipelineAsync(descriptor);
    
    // pipeline for wireframe
    let descriptor2 = ws.createRenderPipelineDescriptor({
        init, shader,
    ...
    // create texture for MASS (for count = 4)
